...
class AdmissionApplication(http.Controller):
    ...
    def admission_registration(self, **get):
        ...
        religion = http.request.env['odoocms.religion'].sudo().search([])
        province = http.request.env['odoocms.province'].sudo().search([])
        ...
        opening_date = http.request.env['odoocms.opening.date'].sudo().search([], limit=1)
        ...
        if not register:
            ...
            return http.request.render("odoocms_admission_portal.admission_close_message", values)
        ...
        Data = {
            ...
            'admission_class_ids': register.program_ids,
        ...
        if application:
            ...
            return request.render("odoocms_admission_portal.application_form_success", vals)
        else:
            values = {
    ...
    @http.route(['/application_registration/submit'], type='http', auth="public", website=True, method=['GET'])
    def application_registration_submit(self, **post):
        current_user = http.request.env.user
        register_id = http.request.env['odoocms.admission.register'].sudo().search([('state', '=', 'application')])
        photo = request.httprequest.files.getlist('photo')
        cnic = post.get('cnic')
        father_cnic = post.get('father_cnic')
        guardian_cnic = post.get('guardian_cnic')

        current_date = datetime.date.today()
        dob = post.get('dob')
        if post.get('dob'):
            age = current_date.year - int(dob.split('-')[0])
            if age > 50:
                dob = current_date

        application = request.env['odoocms.application'].sudo().search(
            [('register_id', '=', register_id.id), ('cnic', '=', cnic)])
        if application:
            values = {
                'header': 'Application Already Exist',
                'message': 'Application with this CNIC is already exist. Kindly verify!',
                'cnic': cnic,
            }
            return http.request.render('odoocms_admission_portal.submission_message', values)

        new_application = request.env['odoocms.application'].sudo().create({
            # Personal Details
            'name': ' '.join([post.get('first_name'), post.get('middle_name'), post.get('last_name')]),
            'first_name': post.get('first_name'),
            'middle_name': post.get('middle_name'),
            'last_name': post.get('last_name'),
            'date_of_birth': dob,
            'gender': post.get('gender'),
            'nationality': post.get('nationality_id'),
            'religion_id': post.get('religion_id'),
            'province_id': post.get('present_province_id') or '',
            'email': post.get('email'),
            'image': base64.b64encode(photo[0].read()),
            'cnic': cnic,
            'mobile': post.get('student_mobile'),
            'is_same_address': bool(post.get('is_same_address')),
            ...
            'father_income': post.get('father_income'),
            'father_mobile': post.get('father_mobile'),

            # Present Country id
            'street': post.get('present_address'),
            'city': post.get('present_city'),
            'street2': post.get('present_province_id'),
            'present_country_id': post.get('country_id'),

            # Permanent Country id
            'per_street': post.get('permanent_address'),
            'per_city': post.get('city_permanent'),
            'per_street2': post.get('permanent_province_id'),
            'per_country_id': post.get('permanent_country_id'),

            # Emergency Contact
            'emergency_name': post.get('emergency_name'),
            'emergency_relation': post.get('relation'),
            'emergency_mobile': post.get('emergency_contact') or '',

            # Guardian Info
            'guardian_name': post.get('guardian_name'),
            'guardian_cnic': guardian_cnic,
            'guardian_occupation': post.get('guardian_occupation'),
            'guardian_relation': post.get('guardian_relation'),

            # Preferences
            'choice_one': post.get('program_id_1') or "",
            'choice_two': post.get('program_id_2') or "",
            'choice_three': post.get('program_id_3') or "",

            # 'cnic': '-'.join([cnic[:5], cnic[5:12], cnic[-1:]]),
            'register_id': register_id.id,
        })
        self.process_matric_data(new_application, post)
        self.process_inter_data(new_application, post)

        attachment_id = request.env['odoocms.application.documents'].sudo().create({
    ...
    @http.route(['/download/admission_form/<int:application_id>'], type='http', auth="public", website=True)
    def portal_my_assignment(self, application_id=None, access_token=None, **kw):
        if application_id:
            application = request.env['odoocms.application'].sudo().browse(application_id)
            pdf = request.env.ref('odoocms_admission_portal.action_applicant_admission_form').sudo()._render_qweb_pdf(
                [application.id])[0]
            pdfhttpheaders = [('Content-Type', 'application/pdf'), ('Content-Length', len(pdf))]
            return request.make_response(pdf, headers=pdfhttpheaders)

    @http.route(['/download/admission_challan_form/<int:application_id>'], type='http', auth="public", website=True)
    def portal_admission_challan_form(self, application_id=None, access_token=None, **kw):
        if application_id:
            application = request.env['odoocms.application'].sudo().browse(application_id)
            application.fee_voucher_state = 'download'
            pdf = \
                request.env.ref('odoocms_admission_portal.action_student_admission_invoice_form').sudo().render_qweb_pdf(
                    [application.id])[0]
            pdfhttpheaders = [('Content-Type', 'application/pdf'), ('Content-Length', len(pdf))]
            return request.make_response(pdf, headers=pdfhttpheaders)

    @http.route('/admission/registration/challan/submit', type='http', auth="user", method=['GET'], website=True)
    def admission_registration_challan_submit(self, **post):
        challan = request.httprequest.files.getlist('challan_img')
        voucher_number = post.get('voucher_number')
        voucher_date = post.get('voucher_date')
        voucher_amount = post.get('voucher_amount')
        current_user = http.request.env.user
        application = http.request.env['odoocms.application'].sudo().search([('cnic', '=', current_user.login)])
        voucher_image_uploaded = application.sudo().write({
            'voucher_image': base64.b64encode(challan[0].read()),
            'voucher_number': voucher_number,
            'voucher_date': voucher_date,
            'voucher_amount': voucher_amount,
        })
        if voucher_image_uploaded:
            application.fee_voucher_state = 'upload0'
            return request.render("odoocms_admission_portal.application_challan_success")
    ...
    #         print(12121212121212121212121212121, guardian_data)
    #         program_preference_data = self.process_program_preference(kw)
    #         print(12121212121212121212121212121, program_preference_data)
    #         data = dict(bdata)
    ...
    #     # print(121212121212121212121212, kw.get('pref_c1'), kw.get('degree'))
    ...
    def process_matric_data(self, application, kw):
        ...
        if kw['matric_degree'] == "Matric":
            ...
            matric_data = {
                ...
                # 'subjects': kw.get('matric_subject'),
                # 'roll_number': kw.get('matric_roll_no'),
                'total_marks': kw.get('o-level-totalmarks'),
                'obtained_marks': kw.get('o-level-obtainedmarks'),
                'percentage': kw.get('o-level-percentage'),
                'result_awaiting': kw.get('matric_result_awaiting'),
            }
    ...
    def process_inter_data(self, application, kw):
        ...
        inter_subject_record = http.request.env['odoocms.degree'].sudo().search([('id', '=', inter_subject)])
        ...
        a_level_totalmarks = kw.get('a-level-totalmarks')
        a_level_obtainedmarks = kw.get('a-level-obtainedmarks')
        a_level_percentage = kw.get('a-level-percentage')
        ...
        inter_data = {
            ...
            'roll_number': kw.get('inter_roll_no'),
            'result_awaiting': kw.get('inter_result_awaiting'),
        }
        ...
        #     return json.dumps(record)
        if kw['inter_degree'] == 'Intermediate':
            ...
            if inter_board and len(inter_board) > 0:
                inter_data['board'] = inter_board
            if a_level_totalmarks and len(a_level_totalmarks) > 0:
                inter_data['total_marks'] = int(a_level_totalmarks)
            if a_level_obtainedmarks and len(a_level_obtainedmarks) > 0:
                inter_data['obtained_marks'] = int(a_level_obtainedmarks)
            if a_level_percentage and len(a_level_percentage) > 0:
                inter_data['percentage'] = float(a_level_percentage)
        ...
        if inter_subject_record:
            inter_data['subjects'] = inter_subject_record.name
            if inter_subject_record.name == 'Pre-Medical':
